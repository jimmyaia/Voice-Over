...
function versionedFilename(filename: string, versionNumber: number) {
  const extensionIndex = filename.lastIndexOf(".");
  const extension = extensionIndex >= 0 ? filename.slice(extensionIndex) : ".mp3";
  const base = extensionIndex >= 0 ? filename.slice(0, extensionIndex) : filename;
  const versionedBase = /_v\d+$/i.test(base)
    ? base.replace(/_v\d+$/i, `_v${versionNumber}`)
  ...
  async function load() {
    ...
    if (versionIds.length) {
      ...
      await Promise.all((versions || []).map(async (version) => {
        ...
        const clip = loadedClips.find((item) => item.current_audio_version_id === version.id);
        const downloadName = versionedFilename(clip?.expected_filename || "voiceover.mp3", version.version_number);
        const { data: downloadData } = await supabase.storage.from("voice-audio").createSignedUrl(version.storage_path, 3600, { download: downloadName });
